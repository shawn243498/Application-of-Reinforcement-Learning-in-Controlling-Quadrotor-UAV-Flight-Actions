...
def plot_with_markers(x, y, T, number):
    ...
    for i in range(len(x)):
        ...
        if i == (len(x)-1):
            ...
            # elif T[i] == 1:
            #     plt.plot(x[i], y[i], marker='o', color='green', markersize=5, zorder=5)
...
# 繪製所有線條
plot_with_markers(x1, y1, T1, number1)
plot_with_markers(x2, y2, T2, number2)
plot_with_markers(x3, y3, T3, number3)
plot_with_markers(x4, y4, T4, number4)
plot_with_markers(x5, y5, T5, number5)
plot_with_markers(x6, y6, T6, number6)
plot_with_markers(x7, y7, T7, number7)
plot_with_markers(x8, y8, T8, number8)
plot_with_markers(x9, y9, T9, number9)
...
plt.yticks(np.arange(-60, 40, 10), fontsize=14)
